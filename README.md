...
Counterfactual confounder adjustment for single-cell differential expression
(Park & Kellis, 2021 CoCoA-diff). Installs the `cocoa` binary.

```sh
cargo install cocoa-rs
```

Optional features: `--features hdf5`, `--features cuda`, `--features metal`.
